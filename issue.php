...
$badgeid = optional_param('id', null, PARAM_ALPHANUM);
...
// Badge has already been selected.
if (!is_null($badgeid)) {
...
// Badge hasn't been selected yet -> show badge selector
else {
    $tree = obf_badge_tree::get_instance();
    $issueurl = new moodle_url('/local/obf/issue.php', array('courseid' => $courseid));
    $content .= $PAGE->get_renderer('local_obf')->render_badge_selector($tree, $issueurl);
}
